...
namespace Plugin.Android.AugustLock
{
    ...
    public class AugustLockDevice
    {
        ...
        private Guid AUGUST_SERVICE = new Guid("0000FE24-0000-1000-8000-00805F9B34FB");
        ...
        internal async Task<IDevice> findLock()
        {
            Debug("Connect()");
            ...
            try
            {
                ...
                try
                {
                    ...
                    {
                        Debug("Discovered: " + a.Device.Id);
                        deviceList.Add(a.Device);
                        if (a.Device.Id == ID)
                        {
                            Debug("Got Lock: " + a.Device.Id);
                            augustLock = a.Device;
                            tokenSource.Cancel();
                        }
                    };
                    ...
                    var scanTask = adapter.StartScanningForDevicesAsync(
                        null, device => device.Id == ID || ID.Equals(Guid.Empty), false, tokenSource.Token);
                ...
                if (augustLock != null)
                {
                    Debug("August Lock found: " + augustLock.Id);
                    return augustLock;
                }
                else
                {
        ...
        private async Task<bool> CompleteConnection()
        {
            var adapter = CrossBluetoothLE.Current.Adapter;
            Debug("Attempt to connect to: " + augustLock.Id);
            var svs = await augustLock.GetServicesAsync();
        ...
        public async Task<Boolean> Toggle()
        {
            Boolean state = true;
            switch (await Status())
            {
